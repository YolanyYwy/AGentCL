...
def run_task(
    ...
    elif issubclass(AgentConstructor, HFAgent):
        # HuggingFace local model agent
        agent = AgentConstructor(
            tools=environment.get_tools(),
            domain_policy=environment.get_policy(),
            **llm_args_agent,  # Pass model_name_or_path, load_in_4bit, etc.
        )
    else:
        raise ValueError(
            f"Unknown agent type: {AgentConstructor}. Should be LLMAgent, LLMSoloAgent, or HFAgent"
        )
    try:
