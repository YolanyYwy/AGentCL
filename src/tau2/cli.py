...
def main():
    ...
    # Continual learning command
    continual_parser = subparsers.add_parser(
        "continual", help="Run continual learning benchmark evaluation"
    )
    ...
    continual_parser.add_argument(
        ...
        help="The domain to run the evaluation on",
    )
    continual_parser.add_argument(
        "--agent-type",
        type=str,
        default="llm_agent",
        choices=["llm_agent", "hf_agent"],
        help="Type of agent: 'llm_agent' (API-based) or 'hf_agent' (local HuggingFace model). Default is 'llm_agent'.",
    )
    continual_parser.add_argument(
        "--agent-llm",
        type=str,
        default=DEFAULT_LLM_AGENT,
        help=f"The LLM to use for the agent (for llm_agent type). Default is {DEFAULT_LLM_AGENT}.",
    )
    continual_parser.add_argument(
        "--model-path",
        type=str,
        default="Qwen/Qwen3-4B",
        help="HuggingFace model path (for hf_agent type). Default is 'Qwen/Qwen3-4B'.",
    )
    continual_parser.add_argument(
        "--load-in-4bit",
        action="store_true",
        default=False,
        help="Load model in 4-bit quantization (for hf_agent type). Default is False.",
    )
    continual_parser.add_argument(
        "--user-llm",
        type=str,
    ...
    continual_parser.add_argument(
        "--training-mode",
        type=str,
        choices=["icl", "vanilla_sft", "none"],
        default="icl",
        help="Training mode: 'icl' (in-context learning), 'vanilla_sft', or 'none'. Default is 'icl'.",
    )
    continual_parser.add_argument(
        "--output-dir",
        "-o",
        type=str,
        default="./continual_results",
        help="Directory to save evaluation results. Default is './continual_results'.",
    )
    continual_parser.add_argument(
        "--num-eval-trials",
        type=int,
        default=4,
        help="Number of evaluation trials per task. Default is 4.",
    )
    continual_parser.add_argument(
        "--max-steps",
        type=int,
        default=DEFAULT_MAX_STEPS,
        help=f"Maximum steps per simulation. Default is {DEFAULT_MAX_STEPS}.",
    )
    continual_parser.add_argument(
        "--seed",
        type=int,
    ...
    continual_parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Print progress information. Default is True.",
    )
    continual_parser.set_defaults(func=lambda args: run_continual_benchmark(args))
...
def run_continual_benchmark(args):
    ...
    if args.agent_type == "hf_agent":
        # Use HuggingFace local model for agent
        agent_llm = None  # Not used for hf_agent
        llm_args_agent = {
            "model_name_or_path": args.model_path,
            "load_in_4bit": args.load_in_4bit,
        }
        # Also use HF model for user (same model, shared cache)
    ...
    if metrics:
        print("\n" + "=" * 60)
        print("CONTINUAL LEARNING BENCHMARK COMPLETE")
        print("=" * 60)
        print(f"Curriculum: {results.curriculum_id}")
        print(f"Stages Completed: {len(results.stage_results)}/{len(runner.curriculum.stages)}")
        print(f"\nKey Metrics:")
